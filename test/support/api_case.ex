defmodule NationalVoterFile.ApiCase do
  @moduledoc """
  This module defines the test case to be used by
  tests that require setting up a connection, specificaly,
  those working with the API endpoints.

  It's basically a clone of NationalVoterFile.ConnCase, with some extras,
  ...
  using(opts) do
    quote do
      ...
      import NationalVoterFile.AuthenticationTestHelpers
      import NationalVoterFile.Router.Helpers
      import NationalVoterFile.Factories
      import NationalVoterFile.TestHelpers

      # The default endpoint for testing
      @endpoint NationalVoterFile.Endpoint

      NationalVoterFile.ApiCase.define_request_helper_methods(unquote(opts))
    end
  end
  ...
  defp add_authentication_headers(conn, true) do
    user = insert(:user)
    conn = conn |> NationalVoterFile.AuthenticationTestHelpers.authenticate(user)
    {conn, user}
  end

  defp add_authentication_headers(conn, :admin) do
    admin = insert(:user, admin: true)
    conn = conn |> NationalVoterFile.AuthenticationTestHelpers.authenticate(admin)
    {conn, admin}
  end
  ...
  defp do_add_request_helper_methods(resource_name) do
    quote do
      ...
      defp path_for(conn, action, resource_or_id) do
        apply(NationalVoterFile.Router.Helpers, path_helper_method(), [conn, action, resource_or_id])
      end

      defp path_for(conn, action) do
        apply(NationalVoterFile.Router.Helpers, path_helper_method(), [conn, action])
      end
      ...
      def request_create(conn, attrs \\ %{}) do
        ...
        payload = NationalVoterFile.JsonAPIHelpers.build_json_payload(attrs)
